...
public class PersonRestClient extends RestClient<PersonClient> implements PersonClient {
	...
	public PersonWithIdResponse createPerson(final AuthValues authValues, Person person) throws WorkflowException {
		...
		if ( logger.isInfoEnabled()) logger.info("Create a new person.");
		
		ClientPerson clientPerson = conversionService.convert(person, ClientPerson.class);
		ResponseEntity<ClientPerson> response = doPost(authValues,ROOT_PATH_SEPERATOR,clientPerson ,ClientPerson.class);
	...
	@Override
	public PersonWithIdResponse updatePerson(final AuthValues authValues, UUID personUuid, Person person) throws WorkflowException {
		
		if ( logger.isInfoEnabled()) logger.info("update a person.");
